...
// import type React from "react"
// import { useEffect, useRef, useState } from "react"
// import { Sparkles, RefreshCw, Trash2, ZoomIn, ZoomOut } from "lucide-react"
// import { Slider } from "@/components/ui/slider"
// import { Card, CardContent } from "@/components/ui/card"
// import { Badge } from "@/components/ui/badge"
// import { Button } from "@/components/ui/button"
...
//   // Canvas dimensions
//   const DISPLAY_SIZE = 400 // Size of visible canvas
//   const MODEL_SIZE = 28 // Size expected by model
//   const SCALE_FACTOR = DISPLAY_SIZE / MODEL_SIZE // For converting between display and model canvas

//   useEffect(() => {
//     // Setup display canvas (what user sees and draws on)
//     const displayCanvas = displayCanvasRef.current
//     if (!displayCanvas) return

//     displayCanvas.width = DISPLAY_SIZE * 2
//     displayCanvas.height = DISPLAY_SIZE * 2
...
//     displayContext.scale(2, 2)
//     displayContext.lineCap = "round"
//     displayContext.strokeStyle = "#000"
...
//     // Setup hidden canvas (for model input, small size)
//     const hiddenCanvas = hiddenCanvasRef.current
//     if (!hiddenCanvas) return

//     hiddenCanvas.width = MODEL_SIZE
//     hiddenCanvas.height = MODEL_SIZE
...
//     hiddenContext.lineWidth = brushSize / SCALE_FACTOR
//     hiddenContextRef.current = hiddenContext

//     hiddenContext.fillStyle = "white"
//     hiddenContext.fillRect(0, 0, MODEL_SIZE, MODEL_SIZE)
...
//     // Start drawing on display canvas
//     if (displayContextRef.current) {
//       displayContextRef.current.beginPath()
//       displayContextRef.current.moveTo(offsetX, offsetY)
//     }
    
//     // Start drawing on hidden canvas
//     if (hiddenContextRef.current) {
//       const scaledX = offsetX / SCALE_FACTOR
//       const scaledY = offsetY / SCALE_FACTOR
//       hiddenContextRef.current.beginPath()
//       hiddenContextRef.current.moveTo(scaledX, scaledY)
//     }
...
//     if (displayContextRef.current) {
//       displayContextRef.current.lineTo(offsetX, offsetY)
...
//       const scaledX = offsetX / SCALE_FACTOR
//       const scaledY = offsetY / SCALE_FACTOR
//       hiddenContextRef.current.lineTo(scaledX, scaledY)
...
//     const displayCanvas = displayCanvasRef.current
//     if (!displayCanvas || !displayContextRef.current) return

//     displayContextRef.current.fillStyle = "white"
//     displayContextRef.current.fillRect(0, 0, displayCanvas.width, displayCanvas.height)
    
//     // Clear hidden canvas
//     const hiddenCanvas = hiddenCanvasRef.current
//     if (!hiddenCanvas || !hiddenContextRef.current) return
    
//     hiddenContextRef.current.fillStyle = "white"
//     hiddenContextRef.current.fillRect(0, 0, MODEL_SIZE, MODEL_SIZE)
...
//   const recognizeDoodle = async () => {
//     if (!hasDrawn || !hiddenCanvasRef.current) return

//     setRecognizing(true)

//     try {
//       // Convert hidden canvas to blob
//       const blob = await new Promise<Blob>((resolve) => {
...
//           {/* Hidden canvas for model input (not visible to user) */}
//           <canvas
//             ref={hiddenCanvasRef}
//             className="hidden"
//           />
          
//           {/* Preview indicator showing the model's view */}
//           <div className="absolute bottom-2 right-2 border border-slate-300 dark:border-slate-700 bg-white p-1 rounded">
//             <div className="text-xs text-slate-500 mb-1 text-center">Model view</div>
//             <div className="relative w-16 h-16 border border-slate-300">
//               {hiddenCanvasRef.current && (
//                 <img 
//                   src={hiddenCanvasRef.current.toDataURL()} 
//                   alt="Model view" 
